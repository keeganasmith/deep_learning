        ...
        for k in datasets[n]:
            if(idx % world_size == local_rank):
                ...
                for epoch in range(epochs):
                    ...
                    print(f"{n}, {k} - Epoch {epoch+1}/{epochs} - Train Loss: {avg_train_loss:.4f} - Val Loss: {avg_val_loss:.4f}, Val log cost: ", sigma)

                print("final best validation sigma: ", sigma)
                sigmas.append(sigma)
                torch.save(best_model_state, f"../models/{n}-{k}_model.pt")
                ...
                plt.grid(True)
                plt.tight_layout()
                plt.savefig(f"../plots/{n}-{k}_training_and_validation_loss.png")
